<template>
	<view>
		<block v-for="(item,index) in list_data" :key="index" v-if="getMinToMax(index)">
			<view class="bg-white margin-top padding radius lc-trends">
				<!--用户信息-->
				...
				<!--内容-->
				<view class="margin-tb text-black lc-text-content" @tap="listTap('contentTap',item,index)">
				...
				<!-- #ifndef APP-PLUS -->
				<view class="lc-video-box" v-if="item.video">
				...
				<view class="lc-img-grid-col" v-if="item.img.length > 0">
					<!--单图-->
				...
				<!--操作-->
				<view class="flex p-xs lc-footer-tool">
...
<script>
	import videoRender from '@/components/lc-common/basics/video-render';
