<template>
	<view>
		...
		<!--tab列表-->
		<view class="bg-white lc-nav-tab-view">
		...
		<block v-for="(item,index) in order_list" :key="index" v-if="tab_cur == 0">
			<view class="bg-white lc-order-tab-view" @tap="detailsTap">
				<!--店铺名称-->
				...
				<!--按钮-->
				<view class="lc-btn-view" v-if="item.type == 1">
				...
				<!--按钮-->
				<view class="lc-btn-view" v-if="item.type == 0">
					<button class="cu-btn line-black sm radius">联系卖家</button>
				</view>
			</view>
		</block>
		...
		<!--无数据-->
		<view class="bg-white lc-null-view" v-if="tab_cur != 0">
			<view class="img-view">
				<view class="cu-avatar lg round" style="background-image:url('/static/lcui/img/aa6.png')"/>
			</view>
		...
		<!--到底了-->
		<view class="lc-foot-tip-view" v-if="tab_cur == 0">
			<view class="img-view">
				<view class="cu-avatar round" style="background-image:url('/static/lcui/img/acn.png')"/>
			</view>
		...
		<!-- #ifdef MP -->
			<!--编辑-->
			<view class="lc-add-btn-view-box" @tap="barEditTap">
...
<script>
	...
	import _sort_grid from '@/static/lcui/data/order_list.js';	//虚拟数据
...
<style lang="scss">
	...
	/* #endif */
	@import "../../static/lcui/style/order-list.scss";
</style>
